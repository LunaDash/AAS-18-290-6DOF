...
class RL_stats(object):
    ...
    def __init__(self,lander,logger,allow_plotting=True,print_every=1, vf=None, scaler=None):
        ...
        self.formats = {'gs_f' : '{:8.1f}' , 'r_f' : '{:8.1f}', 'v_f' : '{:8.2f}' , 'r_i' : '{:8.1f}', 'v_i' : '{:8.2f}' , 'norm_rf' : '{:8.1f}', 'norm_vf' : '{:8.2f}', 'thrust' : '{:8.0f}',
                        ...
                        'fuel' : '{:8.0f}', 'steps'  : '{:8.0f}', 'r_bonus' : '{:8.2f}', 'v_bonus' : '{:8.2f}', 'norm_thrust' : '{:8.0f}', 'a_f' : '{:8.1f}',  'w_f' : '{:8.1f}',
    ...
    def update_episode(self,sum_rewards,steps):    
        ...
        #self.stats['attitude'].append(np.max(np.abs(self.lander.trajectory['attitude']),axis=0))
        #self.stats['w'].append(np.max(np.abs(self.lander.trajectory['w']),axis=0))
        self.stats['attitude'].append(self.lander.trajectory['attitude'])
        ...
        self.stats['a_f'].append(self.lander.trajectory['attitude'][-1][1:3])
        self.stats['w_f'].append(self.lander.trajectory['w'][-1])
        ...
        self.stats['norm_af'].append(np.linalg.norm(self.lander.trajectory['attitude'][-1][1:3]))  # don't care about yaw
