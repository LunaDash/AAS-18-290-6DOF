...
class Lander_model(object):
    ...
    def __init__(self, thruster_model, use_trajectory_list=False, attitude_parameterization=None, divert = (0.0,  0.0, 0.0), 
                 apf_v0=70, apf_tau1=20, apf_tau2=250, gain=1.5, apf_vf1=-2.0, apf_vf2=-1.0, apf_atarg=5.0):  
        self.traj = {}
        self.state_keys = ['position','velocity','thrust','bf_thrust', 'torque', 'attitude', 'w',  'mass', 't_go']
        ...
        self.get_state_agent    = self.get_state_agent1

        self.apf_state = False
        self.trajectory = {}
        self.state = {}
        self.prev_state = {}
        print('Lander Model fixed: ',self.max_thrust)
    ...
    def get_state_agent3(self,t):
        error, t_go = self.track_func(self.state['position'], self.state['velocity'] )
        state = np.hstack( ( error,  self.state['attitude'], self.state['w'] , np.linalg.norm(self.state['position']), np.linalg.norm(self.state['velocity'])))
        return state

    def get_state_agent4(self,t):
        error, t_go = self.track_func(self.state['position'], self.state['velocity'] )
        state = np.hstack( ( error,  self.state['attitude'], self.state['w'] , np.linalg.norm(self.state['position']),  np.linalg.norm(self.state['velocity']), t_go))
        return state
    ...
    def get_state_agent6(self,t):
        error, t_go = self.track_func(self.state['position'], self.state['velocity'] )
        state = np.hstack( ( error,  self.state['attitude'], self.state['w'] , self.state['mass'], t_go))
        return state

    def get_state_agent7(self,t):
        error, t_go = self.track_func(self.state['position'], self.state['velocity'] )
        state = np.hstack( ( error,  self.state['attitude'], self.state['w'] , t_go))
        return state

    def get_state_agent8(self,t):
        error, t_go = self.track_func(self.state['position'], self.state['velocity'] )
        state = np.hstack( ( error,  self.state['attitude'], self.state['w'] , self.state['mass'], t_go))
        return state

    def get_state_agent9(self,t):
        error, t_go = self.track_func(self.state['position'], self.state['velocity'] )
        state = np.hstack( ( error,  self.state['attitude'], self.state['w'] , t_go, self.state['position'][2]))
        return state

    def get_state_agent10(self,t):
        error, t_go = self.track_func(self.state['position'], self.state['velocity'] )
        state = np.hstack( ( error,  self.state['attitude'], self.state['w'] ,  self.state['position'][2]))
        return state

    def get_state_agent11(self,t):
        error, t_go = self.track_func(self.state['position'], self.state['velocity'] )
        state = np.hstack( ( error,  self.state['attitude'], self.state['w'] , t_go, self.state['position'][2], self.state['mass']))
        return state
    ...
    def show_final_stats(self,type='final'):
        ...
        stats = {}
        keys = ['position', 'velocity', 'fuel', 'attitude', 'w', 'glideslope']
        formats = {'position' : '{:8.1f}' , 'velocity' : '{:8.3f}', 'fuel' : '{:6.2f}', 'attitude' : '{:8.3f}', 'w' : '{:8.3f}', 'glideslope' : '{:8.3f}'}
        ...
        for traj in self.trajectory_list:
            for k in keys:
                ...
                if len(v.shape) == 1:
                    v = np.expand_dims(v,axis=1)
                if k == 'attitude':
                    foo = self.attitude_parameterization.q2Euler321(v[idx])
                else:
                    foo = v[idx]
                stats[k].append(foo)
    ...
    def get_engagement_keys(self):
        keys = ['t', 'position', 'velocity', 'attitude', 'w', 'thrust', 'bf_thrust', 'torque', 'mass', 'fuel', 'v_ratio','vc', 'range', 'los', 'dlos', 'zem','reward','fuel_reward','tracking_reward','glideslope_reward', 'landing_margin', 'glideslope', 'sc_margin', 'glideslope_penalty','sc_penalty','sc_reward','att_penalty','att_reward','landing_reward']
        return keys
